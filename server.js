...
async function startServer() {
    ...
    app.get('/api/dashboard/stats', require('./middleware/auth').authMiddleware, (req, res) => {
        ...
        const recentDownloads = db.prepare('SELECT d.*, w.title as work_title FROM downloads d LEFT JOIN works w ON d.work_id = w.id ORDER BY d.downloaded_at DESC LIMIT 5').all();
        const topWorks = db.prepare('SELECT id, title, download_count FROM works WHERE download_count > 0 ORDER BY download_count DESC LIMIT 5').all();
    ...
    app.use((err, req, res, next) => {
        ...
        if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: '文件太大，最大支持 500MB' });
